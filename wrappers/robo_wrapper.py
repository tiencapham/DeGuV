...
class FrameStackWrapper(dm_env.Environment):
    ...
    def _transform_observation(self, time_step):
        ...
        # process depth
        frame = [self._frames[0][:3,:,:], self._frames[1][:3,:,:], self._frames[2][:3,:,:], self._frames[0][3:4,:,:], self._frames[1][3:4,:,:], self._frames[2][3:4,:,:]]
        obs = np.concatenate(list(frame), axis=0)
...
def robo_make(name, frame_stack=3, action_repeat=2, seed=1, scene_id=0, mode_cfg = "train"):
    # create environment instance
    env = robosuitevgb.make_env(task_name=name, seed=seed, scene_id=scene_id, mode_cfg = mode_cfg)
    # add wrappers
    env = Gym2DMC(env)

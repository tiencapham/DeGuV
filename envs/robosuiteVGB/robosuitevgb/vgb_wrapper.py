...
class VGBWrapper(gym.core.Env, Wrapper):
    ...
    def _initialize_modders(self):
        # print(f"[DEBUG] randomize_ color{self.randomize_color}")
        if self.randomize_color:
            # a = [type(comp) for comp in self.env.sim.model.geom_names]
            self.color_randomization_args['geom_names'] = tuple(
                geom for geom in self.env.sim.model.geom_names if geom is not None and 'robot' not in geom and 'gripper' not in geom)
    ...
    def rescale(self, array):
        ...
        if range_val == 0:
            return np.zeros_like(array)  # Return all zeros if array is constant
        else:
    ...
    def _reformat_obs(self, obs_dict, disable_channel_first=False):
        ...
        for name, obs in obs_dict.items():
            if name.endswith("_image") and "rgb" in self.obs_modality:
                ...
                if self._use_depth:
                    ...
                    d_obs = np.round(self.rescale(np.array(depth_obs))*255.0).astype(np.uint8)
                    obs = np.concatenate([obs, d_obs], axis=2)
                obs = np.flipud(obs)
        ...
        if "rgb" in self.obs_modality:
            ...
            if self.video_background:
                if disable_channel_first:
                    reformatted["rgb"] = np.transpose(reformatted["rgb"], (2, 0, 1))
 
                reformatted["rgb"], depth = reformatted["rgb"][:-1,:,:], reformatted["rgb"][-1:,:,:]
                ...
                reformatted["rgb"] = (1 - idx) * reformatted["rgb"] + idx * self.video_buf[self.step_count % self.video_len, :,
                                              :, :]
                reformatted["rgb"] = np.concatenate([reformatted["rgb"], depth], axis = 0)

                if disable_channel_first:
                    reformatted["rgb"] = np.transpose(reformatted["rgb"], (1, 2, 0))
    ...
    def reset(
        ...
        if not self.except_robot:
            ...
            for geom in self.env.sim.model.geom_names:
                # print(f"[DEBUG] {geom}, {self.env.sim.model.geom_names}")
                if (geom is not None) and ('robot' in geom or 'gripper' in geom):
    ...
    def render(self, mode="rgb_array", render_camera=None, backend="cv2", waitkey=100):
        """
        ...
        if mode == "human":
            ...
            rgb_obs = self._reformat_obs(
                self.env._get_observations(), disable_channel_first=True
            )["rgb"]

...
if __name__ == "__main__":
    ...
    while count < num_test_steps:
        ...
        for i in range(steps_per_action):
            ...
            total_action = [0.0, 0.0, 0.0, 3.14159, 0.0, 0.0, 0.0]
            env.step(total_action)
            # Get the gripper state~
            # print(dir(env))
            gripper_state = env._eef_xquat
            print(quaternion_to_euler(gripper_state))
            # delta_pose = env.robots[0].recent_ee_pose.delta
            # print(f"current pose {gripper_state}")
            # print(f"delta pose {delta_pose}")
            print(f"action for step: {total_action}")
            env.render()
            time.sleep(0.01)

...
def euler_to_quaternion(roll, pitch, yaw):
    ...
    q_w = c1 * c2 * c3 - s1 * s2 * s3
    q_x = s1 * c2 * c3 + c1 * s2 * s3
    q_y = c1 * s2 * c3 - s1 * c2 * s3
    q_z = c1 * c2 * s3 + s1 * s2 * c3
...
class Workspace:
    def __init__(self, cfg):
        ...
        work_dir = f'{cfg.model_dir}/{self.agent_name}'
        ...
        if self.cfg.env == 'robosuite':
            assert self.cfg.task_name in ['Door', 'TwoArmPegInHole', 'Lift', 'NutAssemblyRound']
    ...
    def setup(self):
        ...
        if self.cfg.env == 'robosuite':
            ...
            self.eval_env = robo_make(name=self.cfg.task_name, action_repeat=self.cfg.action_repeat, frame_stack=self.cfg.frame_stack, seed=self.cfg.seed, mode_cfg= self.cfg.mode)
        elif self.cfg.env == 'habitat':
    ...
    def robo_eval(self):
        ...
        for i in tqdm(range(1, 101)):
            ...
            while not time_step.last():
                ...
                print(f"action: {euler_to_quaternion(action[3], action[4], action[5])}")
                gripper_state = self.eval_env._eef_xquat
                print(f"delta pose: {gripper_state}")
                if self.agent.att is not None:
                    self.video_recorder.record(self.eval_env, self.agent.att.cpu().numpy())
                else:
                    self.video_recorder.record(self.eval_env)
                total_reward += time_step.reward
                episode_reward += time_step.reward
                step += 1
            f = open("{}/file_{}_{}.txt".format(self.model_work_dir, self.cfg.seed, self.cfg.mode), 'a')
            ...
            if self.level == 'train':
                ...
                if i < 100 and i % 10 == 0:
                    ...
                    if self.cfg.env == 'robosuite':
                        from wrappers.robo_wrapper import robo_make
                        self.eval_env = robo_make(name=self.cfg.task_name, action_repeat=self.cfg.action_repeat, frame_stack=self.cfg.frame_stack, seed=self.cfg.seed, scene_id=count, mode_cfg=self.cfg.mode)
